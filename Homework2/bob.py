...
# for ciphertext and key     
def check_ciphertext_and_key(arg):
    valid_args = ["-c","-k"]
    if (arg not in valid_args):
        print("Please provide a ciphertext and key")   
        exit(0)
           
if (len(sys.argv) < 2):
    print("Please provide whole command line parameters after script name 'bob.py'")
    exit(0)
    ...
    if (len(sys.argv) == 6):
        ...
        for i in range(1,p):
            if (i not in generator_check_list):
                print(f"g = {g} NO (This is NOT a primitive root modulo {p}.)")
                exit(0)
        
        print(f"g = {g} OK (This is a primitve root modulo {p}.)")        
        ...
        # bob randomly generates value 'b' (private key)
        # and calculates public key B
